...
func InitRoutes(ws *wsSrv, r *gin.Engine) *gin.Engine {
	...
	r.GET("/sign-up", func(c *gin.Context) {
		c.File("./web/public/register.html")
	})
	r.GET("/sign-in", func(c *gin.Context) {
		c.File("./web/public/login.html")
	})
	r.GET("/chat", func(c *gin.Context) {
		c.File("./web/public/chat.html")
	})
	r.GET("/dashboard", func(c *gin.Context) {
		c.File("./web/public/index.html")
	})
	r.GET("/assignments", func(c *gin.Context) {
		c.File("./web/public/assignments.html")
	})
	r.GET("/attendance", func(c *gin.Context) {
		c.File("./web/public/attendance.html")
	})
...
func (ws *wsSrv) wsHandler(c *gin.Context) {
	...
	if !ok {
		logrus.Warn("Failed to parse token claims")
		c.AbortWithStatus(http.StatusUnauthorized)
		return
	}
	...
	if userId == "" {
		logrus.Warn("UserId not found in token")
		c.AbortWithStatus(http.StatusUnauthorized)
		return
	}

	logrus.Printf("WebSocket connection authorized for user: %s", userId)
	...
	go ws.readFromClient(conn, userId)
}
...
func (ws *wsSrv) readFromClient(conn *websocket.Conn, userId string) {
	defer func() {
		conn.Close()
		ws.mutex.Lock()
		delete(ws.wsClients, conn)
	...
	for {
		...
		msg.Username = userId // Using userId as username for now
